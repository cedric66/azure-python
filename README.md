...
| `spot_savings.py` | `spot-savings`: FinOps value story for spot adoption. Opens with a `SpotSavingsHeadline` KPI scorecard (realized saving, annualized run-rate, fleet savings rate, ranked cluster leaderboard with bar chart), a `SpotTimeline` actual-vs-counterfactual chart, and a `TopSavers` ranked standings table (with annualized + savings-rate %). Followed by `SavingsProjection`, before/after detail, `ActualVsProjection`, pool-level breakup and raw extracts. Verdicts use a defensible retail-counterfactual method. **By default only clusters with a current spot node pool are included** (crisp presentation view); pass `--include-all-clusters` to restore full-fleet behavior | Cost Mgmt, ARG, public Retail Prices API |
