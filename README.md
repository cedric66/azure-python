...
| `spot_eviction.py` | Spot node eviction risk assessment: per-pool preemption frequency (healthresources VirtualMachinePreempted), VMSS-churn counter (Activity Log), spot_max_price (capacity-reclaim vs price-eviction) discriminator; Scorecard, per-pool RiskAssessment, EvictionSnapshot, ChurnTrend, RemediationGuide, SpotPoolInventory with per-pool HIGH/MED/LOW risk bands | Azure Resource Health, ARG, Activity Log (node-RG VMSS delete/deallocate events) |
