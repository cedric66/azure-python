...
| `spot_savings.py` | `spot-savings`: FinOps value story for spot adoption. Opens with a `SpotSavingsHeadline` KPI scorecard (realized saving, annualized run-rate, fleet savings rate, ranked cluster leaderboard with bar chart), a `SpotTimeline` actual-vs-counterfactual chart, a `TopSavers` ranked standings table (with annualized + savings-rate %), and a `SavingsByEnv` roll-up that groups every cluster into prod vs non-prod tiers (environment is inferred from the cluster/RG name: `-d-`/`-s-`/`-r-`/`-p-`/`-u-` -> dev/sit/dr/prod/uat). Followed by `SavingsProjection`, before/after detail, `ActualVsProjection`, pool-level breakup and raw extracts. Verdicts use a defensible retail-counterfactual method. The full fleet in scope is kept by default; pass `--only-spot-clusters` to restrict to clusters with a current spot node pool | Cost Mgmt, ARG, public Retail Prices API |
