...
| `spot_eviction.py` | Spot node eviction risk assessment, consolidated onto one row per spot pool (**SpotPoolRisk**): pool config, preemption frequency (healthresources VirtualMachinePreempted), per-pool VMSS churn (Activity Log) with a separate cluster-unattributed column, spot_max_price (capacity-reclaim vs price-eviction) discriminator, banded eviction rate (SpotResources), a HIGH/MED/LOW verdict with its reasons and a churn bar chart, and the swap candidate — same-size in-region SKU with a lower band, retail price delta, optional forward-looking Spot Placement Score (`--placement-score`), always with `verify_before_move`. Plus Scorecard, ChurnTrend (per-day ops + line chart), RemediationGuide, and two reference tabs: EvictionRates (self-diagnoses when empty) and RawEvidence (Resource Health ∪ Activity Log, tagged by Source) | Azure Resource Health, ARG (+ SpotResources eviction rates), Activity Log (node-RG VMSS delete/deallocate events), Retail Prices, opt-in Spot Placement Score API |
| `workload_spot_readiness.py` | `workload-spot`: why a workload fails on a spot node pool, and which on-demand workloads are safe to move — read from an offline `kubectl get -o yaml` export bundle, with no cluster access and no Azure scope. 23 rules (SPOT-001…SPOT-064) covering placement (spread key that only exists on spot nodes, targets spot without tolerating its taint), the singleton/PDB-deadlock class (`replicas: 1` + `minAvailable: 1` -> `disruptionsAllowed: 0`, which blocks drain/descheduler/consolidation while giving zero protection against preemption), state (RWO volumes, StatefulSets), grace periods beyond the ~30s preemption notice, and observed-state findings from the status subresource (PDB blocking now, singleton on a spot node now, pods Pending & unschedulable, replicas concentrated on one pool, restart pattern consistent with reclaim). Six verdicts from BLOCKED ON SPOT to SPOT CANDIDATE; every finding tagged `spec` vs `status` so live evidence is distinguishable from theory; a `Coverage` tab separating MISSING files from RBAC-SKIPPED ones. Bundle produced by the read-only `examples/export_workloads.sh` | Kubernetes YAML export only (no Azure, no kubectl at run time) |
